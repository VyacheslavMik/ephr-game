(ns game.editor
  (:require [game.controls :as controls]
            [game.pixi :as pixi]
...
(defn ^:export change-position [ev]
  (let [v (js/parseInt (.. ev -target -value))]
    (if (= (.. ev -target -id) "right-range")
      (swap! context assoc-in [:position :y] (- (:height (camera/world-rectangle)) (camera/view-port-height) v))
      (swap! context assoc-in [:position :x] v))))
...
(defn setup-scroll-bar []
  (let [bottom-range (.querySelector js/document "#bottom-range")
        right-range  (.querySelector js/document "#right-range")]
    (set! (.-max bottom-range)  (- (:width  (camera/world-rectangle)) (camera/view-port-width)))
